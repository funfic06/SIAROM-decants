// Atelier Noir — sala de controle SIAROM: redesign 2025 com sidebar escura e cards modernos.
  ...
  return (
    <div className="admin-login">
      <div className="admin-login-card">
        ...
        <div className="admin-eyebrow"><span /> área reservada</div>
        <h1>Arquivo<br /><em>SIAROM.</em></h1>
        <p>Entre para administrar fragrâncias, disponibilidade e pedidos da curadoria.</p>
        <form onSubmit={submit}>
          <label><span>E-mail administrativo</span><input type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" /></label>
          <label><span>Senha criada no Firebase</span><input type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Sua senha" /></label>
          {error && <div className="admin-error">{error}</div>}
          <button className="admin-primary-button" disabled={busy}>{busy ? "Verificando…" : "Entrar no arquivo"}<ArrowUpRight size={16} /></button>
        </form>
        <Link href="/" className="admin-back-link"><ArrowLeft size={14} /> Voltar ao catálogo</Link>
      </div>
    </div>
  );
}
...
function AdminSidebar({
  ...
  user: User;
  tab: string;
  onTab: (t: "perfumes" | "pedidos" | "clientes" | "remessas") => void;
  onLogout: () => void;
  counts: { pedidos: number; clientes: number; remessas: number };
}) {
  const initials = user.email ? user.email[0].toUpperCase() : "A";

  const navItems: { key: "perfumes" | "pedidos" | "clientes" | "remessas"; icon: React.ReactNode; label: string; badge?: number }[] = [
    { key: "perfumes", icon: <Package size={15} />, label: "Perfumes" },
    { key: "pedidos", icon: <ArrowUpRight size={15} />, label: "Pedidos", badge: counts.pedidos },
    { key: "clientes", icon: <Users size={15} />, label: "Clientes", badge: counts.clientes },
    { key: "remessas", icon: <Archive size={15} />, label: "Remessas", badge: counts.remessas },
  ];
      ...
      <div className="admin-sidebar-footer">
        <div className="admin-user-info">
          ...
          <div className="admin-user-meta">
            ...
            <span className="admin-user-role"><ShieldCheck size={10} /> Admin</span>
          </div>
          <button className="admin-logout-btn" onClick={onLogout} title="Sair">
            <LogOut size={15} />
          </button>
        </div>
      </div>
  ...
  return (
    <section className="admin-editor">
      <div className="admin-editor-heading">
        <div>
          <div className="admin-eyebrow"><span /> {form.id ? "editar ficha" : "nova ficha"}</div>
          <h2>{form.id ? form.name || "Editar perfume" : "Adicionar perfume"}</h2>
        </div>
        <button className="admin-icon-button" onClick={onCancel} aria-label="Fechar editor"><X size={18} /></button>
      </div>
      <div className="admin-form-grid">
        <label><span>Nome do perfume *</span><input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Ex.: Oud Satin Mood" /></label>
        <label><span>Marca *</span><input value={form.brand} onChange={(e) => set("brand", e.target.value)} placeholder="Ex.: Maison Francis Kurkdjian" /></label>
        <label><span>Gênero *</span><select value={form.type} required onChange={(e) => set("type", e.target.value)}><option value="" disabled>Selecione o gênero</option>{genders.map((g) => <option key={g} value={g}>{g}</option>)}</select></label>
        <label><span>Preço por ml (R$)</span><input value={form.pricePerMl} onChange={(e) => set("pricePerMl", e.target.value)} placeholder="Ex.: 5,50" /></label>
        <label><span>Taxa de recave (R$)</span><input value={form.recavePrice} onChange={(e) => set("recavePrice", e.target.value)} placeholder="8,00" /></label>
        <label><span>Total disponível (ml)</span><input type="number" min="0" value={form.totalMl} onChange={(e) => set("totalMl", e.target.value)} placeholder="Ex.: 100" /></label>
        <label className="admin-form-wide"><span>Volumes disponíveis (ml)</span><input value={form.customVolumes} onChange={(e) => set("customVolumes", e.target.value)} placeholder="Ex.: 3, 7, 10, 25" /><small style={{ fontSize: "0.72rem", color: "var(--muted)", marginTop: 3 }}>Opcional. Separe por vírgula. Se vazio, usa 3, 7, 10, 15, 20 e 30 ml.</small></label>
        <label className="admin-form-wide"><span>URL da imagem</span><input type="url" value={form.imageUrl} onChange={(e) => set("imageUrl", e.target.value)} placeholder="https://…" /></label>
        <label className="admin-form-wide"><span>Observação</span><textarea value={form.obs} onChange={(e) => set("obs", e.target.value)} placeholder="Lote, conservação ou observação para a equipe." /></label>
      </div>

      <div className="admin-form-section admin-copy-section">
        <div><span className="admin-form-kicker">Texto da ficha</span><p>Personalize o resumo e a descrição que aparecerão na página do perfume.</p></div>
        <div className="admin-form-grid">
          <label className="admin-form-wide"><span>Resumo curto</span><input value={form.shortDescription} onChange={(e) => set("shortDescription", e.target.value)} placeholder="Ex.: Um floral luminoso e confortável." /></label>
          <label className="admin-form-wide"><span>Descrição principal</span><textarea value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Texto completo da fragrância para a página individual." /></label>
          <label className="admin-form-wide"><span>Entrega e conservação</span><textarea value={form.deliveryText} onChange={(e) => set("deliveryText", e.target.value)} /></label>
          <label className="admin-form-wide"><span>Link de referência (opcional)</span><input type="url" value={form.referenceUrl} onChange={(e) => set("referenceUrl", e.target.value)} placeholder="Ex.: https://www.fragrantica.com.br/perfume/…" /></label>
        </div>
      </div>

      <div className="admin-form-section admin-accord-section">
        <div><span className="admin-form-kicker">Cinco principais acordes</span><p>Preencha até cinco acordes para exibir na página pública.</p></div>
        <div className="admin-accord-grid">
          {form.accords.map((accord, i) => (
            <label key={i}><span>Acorde {i + 1}</span><input value={accord} onChange={(e) => setAccord(i, e.target.value)} placeholder={`Ex.: ${["Amadeirado", "Âmbar", "Floral", "Cítrico", "Almiscarado"][i]}`} /></label>
          ))}
        </div>
      </div>

      <div className="admin-form-section">
        <div><span className="admin-form-kicker">Valores por volume</span><p>Deixe vazio para calcular pelo preço por ml.</p></div>
        <div className="admin-price-grid">
          {(parseVolumeList(form.customVolumes).length ? parseVolumeList(form.customVolumes) : volumes).map((ml) => (
            <label key={ml}><span>{ml} ml</span><input value={form.prices[ml] || ""} onChange={(e) => setPrice(ml, e.target.value)} placeholder={form.pricePerMl ? money(Number.parseFloat(form.pricePerMl.replace(",", ".")) * ml) : "—"} /></label>
          ))}
        </div>
      </div>

      <div className="admin-form-section apc-editor-section">
        <div><span className="admin-form-kicker">Opção APC</span><p>Inclui o frasco e o volume configurado.</p></div>
        <label className="admin-switch apc-toggle">
          <input type="checkbox" checked={form.hasApc} onChange={(e) => set("hasApc", e.target.checked)} />
          <span><strong>Oferecer APC + {form.apcMl || 40} ml</strong><small>O cliente verá esta opção junto aos volumes do perfume.</small></span>
        </label>
        {form.hasApc && (
          <div className="admin-apc-fields">
            <label><span>Volume APC (ml)</span><input type="number" min="1" value={form.apcMl} onChange={(e) => set("apcMl", Number(e.target.value) || 40)} /></label>
            <label><span>Valor do frasco (R$)</span><input value={form.apcFrasco} onChange={(e) => set("apcFrasco", e.target.value)} placeholder="10,00" /></label>
            <label><span>Limite por frasco</span><input type="number" min="1" value={form.apcLimit} onChange={(e) => set("apcLimit", e.target.value || "1")} /></label>
          </div>
        )}
      </div>
      ...
      <label className="admin-switch">
        ...
        <span><strong>Publicar no catálogo</strong><small>Quando desativado, o perfume fica invisível para clientes.</small></span>
      </label>

      <div className="admin-editor-actions">
        <button className="admin-secondary-button" onClick={onCancel}>Cancelar</button>
        <button className="admin-primary-button" disabled={saving || !form.name.trim() || !form.brand.trim() || !form.type} onClick={() => onSave(form)}>
          {saving ? "Salvando…" : "Salvar ficha"}<Check size={16} />
        </button>
      </div>
    </section>
  );
...
function ClienteEditor({ initial, onClose, onSaved }: { initial?: Cliente; onClose: () => void; onSaved: (notice: string) => void }) {
  ...
  return (
    <div className="admin-confirm-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="admin-confirm-dialog" style={{ maxWidth: 440 }} role="dialog" aria-modal="true" aria-labelledby="cliente-editor-title">
        <div className="admin-editor-heading">
          <div><div className="admin-eyebrow"><span /> {initial ? "editar cliente" : "novo cliente"}</div><h2 id="cliente-editor-title">{initial ? "Editar cadastro" : "Cadastrar cliente"}</h2></div>
          <button className="admin-icon-button" onClick={onClose} aria-label="Fechar"><X size={18} /></button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 5 }}><span style={{ fontSize: "0.74rem", fontWeight: 600, color: "var(--text-secondary)" }}>Nome *</span><input style={{ padding: "8px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface-raised)", color: "var(--text-primary)", fontSize: "0.84rem", fontFamily: "inherit", outline: "none" }} value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Maria Silva" autoFocus /></label>
          <label style={{ display: "flex", flexDirection: "column", gap: 5 }}><span style={{ fontSize: "0.74rem", fontWeight: 600, color: "var(--text-secondary)" }}>Telefone / WhatsApp</span><input style={{ padding: "8px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface-raised)", color: "var(--text-primary)", fontSize: "0.84rem", fontFamily: "inherit", outline: "none" }} value={telefone} onChange={(e) => setTelefone(e.target.value)} placeholder="Ex.: 71999990000" type="tel" /></label>
        </div>
        {error && <div className="admin-error" style={{ marginTop: "0.75rem" }}><AlertCircle size={15} style={{ flexShrink: 0 }} /> {error}</div>}
...
// ─── Aba de clientes ──────────────────────────────────────────────────────────
function ClientesTab({ clientes, onAdd, onEdit, onDelete, query, onQuery }: { clientes: Cliente[]; onAdd: () => void; onEdit: (c: Cliente) => void; onDelete: (c: Cliente) => void; query: string; onQuery: (q: string) => void }) {
    ...
    <section className="admin-section">
      ...
      {visible.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {visible.map((c) => (
            <div key={c.id} className="admin-cliente-row">
              <div>
                <div className="admin-cliente-name">{c.nome}</div>
                <div className="admin-cliente-meta">{c.telefone || "Sem telefone"} · desde {formatDate(c.criadoEm).split(",")[0]}</div>
              </div>
              <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                <button onClick={() => onEdit(c)} style={{ background: "none", border: "1px solid var(--border)", cursor: "pointer", color: "var(--muted)", padding: "5px 8px", borderRadius: "var(--radius-sm)", display: "flex", alignItems: "center", transition: "all 0.15s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "var(--accent)"; e.currentTarget.style.borderColor = "rgba(91,63,219,0.3)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "var(--muted)"; e.currentTarget.style.borderColor = "var(--border)"; }} title="Editar cliente"><Edit3 size={14} /></button>
                <button onClick={() => onDelete(c)} style={{ background: "none", border: "1px solid var(--border)", cursor: "pointer", color: "var(--muted)", padding: "5px 8px", borderRadius: "var(--radius-sm)", display: "flex", alignItems: "center", transition: "all 0.15s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "var(--danger)"; e.currentTarget.style.borderColor = "rgba(220,38,38,0.3)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "var(--muted)"; e.currentTarget.style.borderColor = "var(--border)"; }} title="Excluir cliente"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
...
function OrderCreator({ perfumes, clientes, onClose, onCreated }: { perfumes: Record<string, unknown>[]; clientes: Cliente[]; onClose: () => void; onCreated: (notice: string) => void }) {
  ...
  const availableVolumes = useMemo(() => {
    ...
    const custom = Array.isArray(selectedPerfume.customVolumes) ? (selectedPerfume.customVolumes as unknown[]).map(Number).filter(Boolean) : [];
    return custom.length ? custom : volumes;
  }, [selectedPerfume]);
  const selectedCliente = clientes.find((c) => c.id === clienteId);

  const selectStyle = { padding: "8px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface-raised)", color: "var(--text-primary)", fontSize: "0.84rem", fontFamily: "inherit", outline: "none", width: "100%" };
  const labelStyle = { display: "flex" as const, flexDirection: "column" as const, gap: 5 };
  const spanStyle = { fontSize: "0.74rem", fontWeight: 600, color: "var(--text-secondary)" };

  const confirm = async () => {
    if (!clienteId || !perfumeId || !volumeMl) { setError("Selecione o cliente, o perfume e o volume."); return; }
    setSaving(true); setError("");
    try {
      ...
      await createCustomerOrder({ perfumeId, perfumeName: text(perfume.name), brand: text(perfume.brand), volumeMl: volNum, quantity: Number(quantity) || 1, unitPrice: pricePerMl * volNum, customerName: selectedCliente!.nome, contact: selectedCliente!.telefone, payment, status: "novo", createdAt: new Date().toISOString(), source: "catalogo", isApc: false });
  ...
  return (
    <div className="admin-confirm-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="admin-confirm-dialog" style={{ maxWidth: 520 }} role="dialog" aria-modal="true" aria-labelledby="order-creator-title">
        <div className="admin-editor-heading">
          <div><div className="admin-eyebrow"><span /> novo pedido</div><h2 id="order-creator-title">Registrar pedido</h2></div>
          <button className="admin-icon-button" onClick={onClose} aria-label="Fechar"><X size={18} /></button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <label style={labelStyle}><span style={spanStyle}>Cliente *</span><select style={selectStyle} value={clienteId} onChange={(e) => setClienteId(e.target.value)}><option value="" disabled>Selecione o cliente</option>{clientes.map((c) => <option key={c.id} value={c.id}>{c.nome}{c.telefone ? ` — ${c.telefone}` : ""}</option>)}</select></label>
          <label style={labelStyle}><span style={spanStyle}>Perfume *</span><select style={selectStyle} value={perfumeId} onChange={(e) => { setPerfumeId(e.target.value); setVolumeMl(""); }}><option value="" disabled>Selecione o perfume</option>{perfumes.filter((p) => p.available !== false).map((p) => <option key={text(p.id)} value={text(p.id)}>{text(p.name)} — {text(p.brand)}</option>)}</select></label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={labelStyle}><span style={spanStyle}>Volume (ml) *</span><select style={selectStyle} value={volumeMl} onChange={(e) => setVolumeMl(e.target.value)} disabled={!perfumeId}><option value="" disabled>Selecione</option>{availableVolumes.map((v) => <option key={v} value={v}>{v} ml</option>)}</select></label>
            <label style={labelStyle}><span style={spanStyle}>Quantidade</span><input style={{ ...selectStyle }} type="number" min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></label>
          </div>
          <label style={labelStyle}><span style={spanStyle}>Forma de pagamento</span><select style={selectStyle} value={payment} onChange={(e) => setPayment(e.target.value)}><option value="pix">Pix</option><option value="cartao_credito">Cartão de crédito</option></select></label>
        </div>
        {error && <div className="admin-error" style={{ marginTop: "0.75rem" }}><AlertCircle size={15} style={{ flexShrink: 0 }} /> {error}</div>}
        <div className="admin-confirm-actions" style={{ marginTop: "1.25rem" }}>
          <button className="admin-secondary-button" onClick={onClose}>Cancelar</button>
          <button className="admin-primary-button" disabled={saving || !clienteId || !perfumeId || !volumeMl} onClick={() => void confirm()}>
...
function RemessaCreator({ orders, remessas, onClose, onCreated }: { orders: (CustomerOrder & { legacy?: boolean })[]; remessas: Remessa[]; onClose: () => void; onCreated: (notice: string) => void }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const summaries = selectedOrders.map((o) => ({ orderId: o.id!, perfumeName: o.perfumeName, volumeMl: o.volumeMl, isApc: o.isApc }));
  ...
  return (
    <div className="admin-confirm-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="admin-confirm-dialog" style={{ maxWidth: 560 }} role="dialog" aria-modal="true" aria-labelledby="remessa-title">
        <div className="admin-editor-heading">
          <div><div className="admin-eyebrow"><span /> nova remessa</div><h2 id="remessa-title">Selecionar pedidos da caixa</h2></div>
          <button className="admin-icon-button" onClick={onClose} aria-label="Fechar"><X size={18} /></button>
        </div>
        {activeOrders.length === 0 ? (
          <p style={{ color: "var(--muted)", padding: "1rem 0", fontSize: "0.85rem" }}>Nenhum pedido disponível para agrupar.</p>
        ) : (
          <>
            <p style={{ fontSize: "0.82rem", color: "var(--muted)", marginBottom: "0.75rem" }}>Marque os pedidos desta caixa. Só é possível agrupar pedidos do <strong>mesmo cliente</strong>.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", maxHeight: 360, overflowY: "auto" }}>
              {Array.from(ordersByCustomer.entries()).map(([clienteName, clienteOrders]) => (
                <div key={clienteName}>
                  <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.4rem" }}>{clienteName}</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                    {clienteOrders.map((order) => (
                      <label key={order.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0.75rem", borderRadius: "var(--radius-sm)", cursor: "pointer", background: selected.has(order.id!) ? "var(--accent-soft)" : "var(--surface-raised)", border: "1px solid", borderColor: selected.has(order.id!) ? "rgba(91,63,219,0.3)" : "var(--border)", transition: "all 0.15s" }}>
                        <input type="checkbox" checked={selected.has(order.id!)} onChange={() => toggle(order.id!)} style={{ accentColor: "var(--accent)", width: 16, height: 16, flexShrink: 0 }} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: "0.88rem", color: "var(--text-primary)" }}>{order.perfumeName}</div>
                          <div style={{ fontSize: "0.78rem", color: "var(--muted)" }}>{order.isApc ? `APC + ${order.volumeMl} ml` : `${order.volumeMl} ml`}{order.quantity > 1 ? ` · x${order.quantity}` : ""}{order.payment ? ` · ${paymentLabel(order.payment)}` : ""}</div>
                        </div>
                      </label>
                    ))}
                  </div>
            ...
            {selected.size > 0 && <div style={{ marginTop: "0.75rem", fontSize: "0.8rem", color: "var(--text-secondary)" }}><strong>{selected.size}</strong> pedido(s) selecionado(s){customerName && <> · <strong>{customerName}</strong></>}</div>}
          </>
        )}
        {error && <div className="admin-error" style={{ marginTop: "0.75rem" }}><AlertCircle size={15} style={{ flexShrink: 0 }} /> {error}</div>}
        <div className="admin-confirm-actions" style={{ marginTop: "1.25rem" }}>
          <button className="admin-secondary-button" onClick={onClose}>Cancelar</button>
          <button className="admin-primary-button" disabled={selected.size === 0 || saving} onClick={() => void confirm()}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      {remessas.map((remessa) => {
        ...
        return (
          <article key={remessa.id} className="admin-remessa-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.75rem", marginBottom: "0.5rem" }}>
              <div>
                <strong style={{ fontSize: "0.92rem", color: "var(--text-primary)" }}>{remessa.customerName}</strong>
                <div style={{ fontSize: "0.76rem", color: "var(--muted)", marginTop: 2 }}>{formatDate(remessa.createdAt)} · {remessa.orderIds.length} pedido(s)</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
                <select value={status} onChange={(e) => void onStatusChange(remessa.id!, e.target.value as RemessaStatus)}
                ...
                <button onClick={() => onDelete(remessa.id!, remessa.customerName)} title="Excluir remessa"
                  ...
                  aria-label={`Excluir remessa de ${remessa.customerName}`}><Trash2 size={14} /></button>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              {remessa.orderSummaries.map((s) => (
                <div key={s.orderId} style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>· {s.perfumeName} — {s.isApc ? `APC + ${s.volumeMl} ml` : `${s.volumeMl} ml`}</div>
              ))}
            </div>
          </article>
        );
...
export default function AdminPage() {
  ...
  if (!authorized) return (
    <div className="admin-login">
      <div className="admin-login-card">
        <div className="admin-seal"><img src={logoMark} alt="" /></div>
        <div className="admin-eyebrow"><span /> acesso pendente</div>
        <h1>Quase lá.</h1>
        <p>{authMessage || "Usuário não autorizado."}</p>
        <div className="admin-security-note"><ShieldCheck size={18} /><span>Crie um documento em <strong>admins/{user.uid}</strong> com <strong>role: "admin"</strong>.</span></div>
        <button className="admin-secondary-button full" onClick={() => void signOutAdmin()}>Sair</button>
        <Link href="/" className="admin-back-link"><ArrowLeft size={14} /> Voltar ao catálogo</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="admin-shell">
      ...
      <AdminSidebar
        ...
        onLogout={() => void signOutAdmin()}
        counts={{ pedidos: orders.length + legacyOrders.length, clientes: clientes.length, remessas: remessas.length }}
      />
      ...
      <div className="admin-main">
        ...
        <div className="admin-topbar">
          <span className="admin-topbar-title">{tabTitle[tab]}</span>
          <div className="admin-topbar-right">
            <button className="admin-secondary-button" onClick={() => void recalibrateAvailability()} disabled={recalibrating || !ordersReady} style={{ fontSize: "0.78rem", padding: "6px 12px" }}>
              <RefreshCw size={13} /> {recalibrating ? "Recalibrando…" : "Recalibrar"}
            </button>
            <div className="admin-live-status">
              <span className={isLive ? "live-dot" : "warning-dot"} />
              {isLive ? "sincronizado" : "verificando"}
            </div>
          </div>
        </div>
        ...
        <div className="admin-content">
          ...
          <div className="admin-tabs">
            <button className={tab === "perfumes" ? "active" : ""} onClick={() => { setTab("perfumes"); setQuery(""); }}><Package size={14} /> Perfumes</button>
            <button className={tab === "pedidos" ? "active" : ""} onClick={() => { setTab("pedidos"); setQuery(""); }}><ArrowUpRight size={14} /> Pedidos <span>{orders.length + legacyOrders.length}</span></button>
            <button className={tab === "clientes" ? "active" : ""} onClick={() => { setTab("clientes"); setQuery(""); }}><Users size={14} /> Clientes <span>{clientes.length}</span></button>
            <button className={tab === "remessas" ? "active" : ""} onClick={() => { setTab("remessas"); setQuery(""); }}><Archive size={14} /> Remessas <span>{remessas.length}</span></button>
          </div>
                ...
                {visiblePerfumes.length === 0 ? (
                  ...
                  return (
                    <article className="admin-perfume-row" key={perfumeId}>
                      <div className="admin-perfume-image">
                        {text(raw.imageUrl) ? <img src={text(raw.imageUrl)} alt="" /> : <img src={logoMark} alt="" />}
                      </div>
                      <div className="admin-perfume-info">
                        ...
                        <div className="admin-row-meta">
                          ...
                          <span>{raw.available === false ? "Não aparece" : "Visível no catálogo"}</span>
                        </div>
                        <div className="admin-row-actions">
                          <button onClick={() => setEditing(formFromRaw(raw))}><Edit3 size={13} /> Editar</button>
                          <button onClick={() => setMessagePerfume(raw)}><MessageCircle size={13} /> Mensagem</button>
                          <button onClick={() => void toggleAvailability(raw)}>{raw.available === false ? <><Eye size={13} /> Publicar</> : <><EyeOff size={13} /> Ocultar</>}</button>
                          <button className="danger" onClick={() => void removePerfume(perfumeId)}><Trash2 size={13} /> Remover</button>
                        </div>
                      </div>
                    </article>
                  );
            ...
            <section className="admin-section">
              <div className="admin-section-heading">
                ...
                <div className="admin-header-actions">
                  {legacyOrders.length > 0 && <button className="admin-secondary-button" onClick={() => void migrateLegacyOrders()} disabled={migrating}><RefreshCw size={14} /> {migrating ? "Migrando…" : "Migrar legados"}</button>}
                  {removableOrderCount > 0 && <button className="admin-secondary-button admin-clear-button" onClick={requestClearRemovable}><Trash2 size={14} /> Limpar cancelados</button>}
                  <button className="admin-secondary-button" onClick={() => window.location.reload()}><RefreshCw size={14} /> Atualizar</button>
                  <button className="admin-primary-button" onClick={() => setOrderCreatorOpen(true)} disabled={clientes.length === 0} title={clientes.length === 0 ? "Cadastre um cliente antes de registrar pedidos" : undefined}><UserPlus size={14} /> Novo pedido</button>
                </div>
              </div>
              {clientes.length === 0 && (
                <div className="admin-error" style={{ marginBottom: "1rem" }}>
                  <AlertCircle size={15} /> Nenhum cliente cadastrado. Vá até a aba <strong>Clientes</strong> e cadastre um antes de registrar pedidos.
                </div>
              )}
              ...
              <div className="admin-orders-table">
                ...
                {visibleOrders.length === 0 ? (
                  <div className="admin-empty" style={{ borderRadius: 0, border: "none", borderTop: "1px solid var(--border)" }}>Nenhum pedido encontrado.</div>
                  ...
                  return (
                    <div className="admin-order-row" key={order.id} style={{ opacity: isCanceled ? 0.55 : 1 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                        {isCanceled ? (
                          <span style={{ fontSize: "0.74rem", padding: "2px 8px", borderRadius: 99, background: "var(--danger-soft)", color: "var(--danger)", fontWeight: 600, width: "fit-content" }}>Cancelado</span>
                        ) : emRemessa && remessa ? (
                          <span style={{ fontSize: "0.74rem", padding: "2px 8px", borderRadius: 99, background: "var(--accent-soft)", color: "var(--accent)", fontWeight: 500, width: "fit-content", display: "flex", alignItems: "center", gap: 4 }}><Archive size={11} />{remessa.customerName} · {remessaStatusLabel(remessa.status || "confirmado")}</span>
                        ) : (
                          <span style={{ fontSize: "0.74rem", padding: "2px 8px", borderRadius: 99, background: "var(--warning-soft)", color: "var(--warning)", fontWeight: 500, width: "fit-content", display: "flex", alignItems: "center", gap: 4 }}><AlertCircle size={11} /> Sem remessa</span>
                        )}
                        {!isCanceled && !order.legacy && (
                          <button onClick={() => void handleCancelOrder(order)} style={{ fontSize: "0.72rem", color: "var(--danger)", background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0, width: "fit-content", opacity: 0.7 }} onMouseEnter={(e) => (e.currentTarget.style.opacity = "1")} onMouseLeave={(e) => (e.currentTarget.style.opacity = "0.7")}>Cancelar pedido</button>
                        )}
                      </div>
                    </div>
                  );
          ...
          {!editing && tab === "clientes" && (
            <ClientesTab clientes={clientes} onAdd={() => setClienteEditorTarget("new")} onEdit={(c) => setClienteEditorTarget(c)} onDelete={(c) => setDeleteClienteTarget(c)} query={query} onQuery={setQuery} />
          )}
          ...
          {!editing && tab === "remessas" && (
            <section className="admin-section">
              <div className="admin-section-heading">
                ...
                <button className="admin-primary-button" onClick={() => setRemessaCreatorOpen(true)}><Archive size={16} /> Nova remessa</button>
              </div>
      ...
      {clienteEditorTarget && (
        <ClienteEditor initial={clienteEditorTarget === "new" ? undefined : clienteEditorTarget} onClose={() => setClienteEditorTarget(null)} onSaved={(msg) => { setClienteEditorTarget(null); setNotice(msg); }} />
      )}
      ...
      {messagePerfume && (
        <div className="admin-confirm-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setMessagePerfume(null)}>
          <div className="admin-confirm-dialog admin-message-dialog" role="dialog" aria-modal="true" aria-labelledby="whatsapp-title">
            <div className="admin-editor-heading">
              <div><div className="admin-eyebrow"><span /> mensagem pronta</div><h2 id="whatsapp-title">{text(messagePerfume.name) || "Perfume"}</h2></div>
      ...
      {orderCreatorOpen && (
        <OrderCreator perfumes={rawPerfumes} clientes={clientes} onClose={() => setOrderCreatorOpen(false)} onCreated={(msg) => { setOrderCreatorOpen(false); setNotice(msg); }} />
      )}
    </div>
  );
}
