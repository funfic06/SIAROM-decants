...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (!authorized) { setOrdersReady(false); return; }
    return subscribeOrders((next) => { setOrders(next); setOrdersReady(true); }, (error) => setOrdersError(error.message));
  }, [authorized]);
  ...
  const requestClearDelivered = () => { if (deliveredOrderCount > 0) setClearDialogOpen(true); };
  const clearDeliveredOrders = async () => {
    if (!deliveredOrderCount) return;
    try {
      const remoteIds = orders.filter((order) => order.status === "entregue" && order.id).map((order) => order.id as string);
      const legacyIds = new Set(legacyOrders.filter((order) => order.status === "entregue").map((order) => order.id));
      ...
      setClearDialogOpen(false); setNotice(`${remoteIds.length + legacyIds.size} pedido(s) entregue(s) removido(s).`);
    } catch { setClearDialogOpen(false); setOrdersError("Não foi possível limpar os pedidos entregues. Nenhum pedido ativo foi alterado."); }
  };
